...
import { createFileWatcher, regenerateAllUIKit, cleanup } from './file-watcher.js';
 ...
 */
async function findUIKitMLFiles(sourceDir, include, exclude, verbose = false) {
    const files = [];
    ...
    try {
        ...
        if (verbose) {
            console.warn(`[compile-uikitml] Error scanning directory ${sourceDir}:`, error);
        }
    }
    return files;
}
 ...
 */
async function compileUIKitMLFile(sourcePath, outputPath, verbose = false) {
    ...
    try {
        ...
        if (verbose) {
            ...
            console.log(`[compile-uikitml] File content length: ${sourceContent.length} characters`);
        }
        let parseResult;
        try {
            ...
            parseResult = parse(sourceContent, {
                onError: (message) => {
                    console.error(`[compile-uikitml] Parse error in ${sourcePath}: ${message}`);
                },
            });
        }
        catch (error) {
        ...
        if (verbose) {
            console.log(`[compile-uikitml] ✅ Compiled: ${fileName} -> ${path.basename(outputPath)}`);
        }
        return {
            fileName,
            success: true,
 ...
 */
async function compileAllUIKitMLFilesBuild(pluginOptions, compilationStats) {
    ...
    console.log(`[compile-uikitml] Found ${files.length} .uikitml files to compile`);
    // Process each file
    for (const filePath of files) {
        const relativePath = path.relative(sourceRoot, filePath);
        const outputPath = path.join(outputRoot, relativePath.replace(/\.uikitml$/, '.json'));
    ...
    if (verbose) {
        ...
        console.log(`[compile-uikitml] ✅ Compilation complete: ${successCount} success, ${failureCount} failed`);
        if (failureCount > 0) {
 ...
 */
async function generateInitialUIKit(pluginOptions) {
    ...
    return {
        ...
        configResolved(resolvedConfig) {
            ...
            if (pluginOptions.verbose) {
                console.log(`[compile-uikitml] Plugin initialized with options:`, pluginOptions);
        ...
        async configureServer(_server) {
            ...
            const gate = async (_req, _res, next) => {
                await initialReady;
                next();
            };
            // Prefer to prepend gate to the middleware stack so it runs first
            const stack = _server?.middlewares?.stack;
            ...
            watcher = createFileWatcher(watchDir, pluginOptions, () => devServer?.ws.send({ type: 'full-reload' }));
            console.log('👁️  UIKitML file watcher started - monitoring for changes...');
        },
        async buildStart() {
        ...
        closeBundle: {
            async handler() {
                ...
                if (compilationStats.length > 0) {
                    const successfulCompilations = compilationStats.filter((stat) => stat.success);
                    const failedCompilations = compilationStats.filter((stat) => !stat.success);
